...
- API 26–28 without `WRITE_EXTERNAL_STORAGE` reports gallery-not-available / SAF routing (`ExportInstrumentedTest.galleryWithoutWriteExternalStorageRoutesToSaf`, API 26 cell). This asserts `galleryUsesMediaStore` is false and that `exportToGallery` throws `UNSUPPORTED_DESTINATION`. It does **not** launch `ExportPlugin` or complete `ACTION_CREATE_DOCUMENT` UI. Plugin SAF dispatch/picker UI stays UNVERIFIED. `createDocumentIntent` / `completeSafExport` (file://) are the exercised halves of that flow.
