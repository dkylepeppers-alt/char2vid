...
- [x] Run `npx vitest run tests/contract/character-revisions.test.ts` and `npx playwright test tests/e2e/characters.spec.ts`. Test separate outfits, rejected views, missing local source files, and character export/import using G4. Evidence on merge tip `379d3fe` / PR tip `4da7738` (Node from `.nvmrc`): `character-revisions` 14/14; suite 196/196; Playwright characters + CI web/APK/emulators green on the Ready tip. Missing local source covered in contracts. Android character packages now share schema v2 with web (domain `ARCHIVE_SCHEMA_VERSION_WITH_CHARACTERS`, JVM `ArchiveJson`/`ArchivePaths` tests). Physical-device package restore remains **UNVERIFIED**. [#10](https://github.com/dkylepeppers-alt/char2vid/issues/10) is now closed; C2 compile/slot-attach is on `main` via [#51](https://github.com/dkylepeppers-alt/char2vid/pull/51)/[#58](https://github.com/dkylepeppers-alt/char2vid/pull/58).
...
- [x] Write a capacity test that makes silent reference dropping impossible:
...
```
  ...
  Evidence: `npx vitest run tests/contract/reference-plan.test.ts tests/contract/prompt-compiler.test.ts tests/contract/request-snapshot.test.ts` — 24 passed on tip `f6277cb` ([#51](https://github.com/dkylepeppers-alt/char2vid/pull/51)).
- [x] Add tests for stable ordering, same-name characters, one-image-only video routes, input/output counts, format limits, and accepted prompt text with Unicode. Verify identity notes and outfit notes are not automatically injected as repeated appearance descriptions. Covered in `tests/contract/reference-plan.test.ts` and `tests/contract/prompt-compiler.test.ts` on tip `f6277cb` ([#51](https://github.com/dkylepeppers-alt/char2vid/pull/51)).
- [x] Implement deterministic role assignment and user-resolvable omissions. The role plan precedes media staging; prepared inputs retain revision hash/ordinal. Resolve references from library files afresh for every generation; never store a provider URL as an identity reference. ([#51](https://github.com/dkylepeppers-alt/char2vid/pull/51); Create stages `plan.selected` only; `freezeRequest` / `identitiesForStaging` keep hashes and reject provider URLs)
- [x] Implement prompt modules for subject bindings, requested change/action, scene, camera, lighting, style, and output intent. Let users expand/edit/remove modules. Persist raw text, accepted compiled text, and module versions. Prompt assistance cannot silently replace text at Generate time. ([#51](https://github.com/dkylepeppers-alt/char2vid/pull/51); Generate uses `textForGenerate` with `applyProposal` default false)
- [x] Run `npx vitest run tests/contract/reference-plan.test.ts tests/contract/prompt-compiler.test.ts`; commit: `feat: compile stable character references and generation prompts` (`4b28ce5`; follow-ups on [#51](https://github.com/dkylepeppers-alt/char2vid/pull/51) for Create UI, generated-slot attach, Android available-only character packages, and Prompt locator uniqueness). Evidence on tip `f6277cb` (Node `v24.19.0`): focused vitest 24 passed; `npm test` 223 passed; `CI=1 npm run test:e2e` 15 passed including `tests/e2e/create-compiler.spec.ts`. Physical-device generation and paid Nano-GPT remain UNVERIFIED.
