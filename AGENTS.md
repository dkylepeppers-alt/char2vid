...
- Cloud `gh` defaults to the `cursor` integration token (`ghs_`) from `~/.config/gh/hosts.yml` and cannot write issues or PRs (`Resource not accessible by integration`). For issue/PR writes (`gh issue comment`, `gh pr create`/`edit` when ManagePullRequest is unavailable), `export GH_TOKEN="$Cursor"` first. The environment secret is named `Cursor`; `GH_TOKEN` takes precedence over `GITHUB_TOKEN` for the GitHub CLI. Never echo or commit that value, and do not put it in `VITE_*` or the frontend bundle. Keep read-only `gh` as-is when it already works.
